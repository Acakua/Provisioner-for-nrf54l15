...
MEMORY
    {
    FLASH (rx) : ORIGIN = 0x48000, LENGTH = 0x11b000
    RAM (wx) : ORIGIN = 0x20013000, LENGTH = 0x1c000
 ...
 *(".kernel.*")
 ...
 net_buf_pool_area : ALIGN_WITH_INPUT { _net_buf_pool_list_start = .; KEEP(*(SORT_BY_NAME(._net_buf_pool.static.*))); _net_buf_pool_list_end = .; } > RAM AT > FLASH
    __data_region_end = .;
bt_mesh_scene_entries_sections : ALIGN_WITH_INPUT
{
 *(SORT_BY_NAME("._bt_mesh_scene_entry.static.*"));
} > FLASH
bt_mesh_sensor_types_sections : ALIGN_WITH_INPUT
{
 ...
 *(".kernel_noinit.*")
    ...
    __kernel_ram_end = 0x20013000 + 0x1c000;
